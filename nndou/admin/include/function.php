   ...
   //打印数组
    ...
    function thumb($img_addr,$width,$hight,$path='',$filename=''){
       ...
       function login($username,$pwd){
         $conn=db_conn();
         $sql="SELECT `admin_id`,`admin_name`,`admin_last_login`,`admin_pwd` FROM  nnd_admin WHERE `admin_name` = '{$username}'";
         ...
         if($result->num_rows>0){
             ...
             if($userinfo['admin_pwd']==md5($pwd)){
                ...
                // setcookie('lastlogin',$userinfo['admin_last_login'],time()+60*10,'/');
                  $_SESSION['lastlogin']=$userinfo['admin_last_login'];
                 //用户名
                 //setcookie('username',$userinfo['admin_name'],time()+60*10,'/');
                  $_SESSION['username']=$userinfo['admin_name'];
                 //设置登录状态
