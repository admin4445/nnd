 <?php 
    include 'header.php';
     ...
     /*删除操作********************************************************************/
      if(!empty($_GET['navid'])&&isset($_GET['navid'])){
          //获取get方式提交的数据
          $id=$_GET['navid'];
          //删除数据的sql
          $sql="DELETE FROM nnd_nav WHERE nav_id=$id";
          ...
          //每一页显示页码的条数
           $size='5';
          //查询总共的条数
          $sqls = "SELECT * FROM nnd_nav";  
          //获取长度
          $count=$conn->query($sqls)->num_rows;
          //获得页码的长度
          $pagecount=ceil($count/$pagelimit);
          //获取页码
          if(!empty($_GET['nav_id']) && isset($_GET['nav_id'])){
              $page = $_GET['nav_id'];
          ...
          $cc=page($page,$count,$size,$pagecount,'nav_id');

          //偏移量
          $n = ($page - 1) * $pagelimit;
          $sql='SELECT * FROM nnd_nav  limit '."$n,"."$pagelimit";
        ...
        <div class="col-md-12">
			<div class="panel">
                <div class="panel-heading">
                  <div class="panel-title">导航条列表</div>
                ...
                <div class="panel-body">
                  ...
                  <table class="table table-striped table-bordered table-hover dataTable">
                      ...
                      <?php foreach($navinfo as $v){?>
                    	<tr class="success">
                        <td class="text-center"><input type="checkbox" value="<?php echo $v['nav_id'] ?>" name="idarr[]" class="cbox"></td>
                        <td><?php echo $v['nav_name'] ?></td>
                        <td><?php echo $v['nnd_url'] ?></td>
                        <td>
		                      <div class="btn-group">
		                        <a href="nav_edit.php?edit=<?php echo $v['nav_id']; ?>" class="btn btn-default btn-gradient"><span class="glyphicons glyphicon-pencil"></span></a>
		                        <a onclick="return confirm('确定要删除吗？');" href="nav_list.php?navid=<?php echo $v['nav_id'];?>" class="btn btn-default btn-gradient dropdown-toggle"><span class="glyphicons glyphicon-trash"></span></a>
		                      </div>
