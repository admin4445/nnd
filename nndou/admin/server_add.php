    <?php 
        ...
        //查询服务类型
        $sqls='SELECT `serv_type_id`, `serv_type_name` FROM nnd_serv_type';
        $servtypes = type($sqls);
        //添加咨询
        if(isset($_POST['sub'])){
            $type = $_POST['serv_type'];
            $name = $_POST['ser_type_name'];
            $ename = $_POST['ser_type_ename'];
            ...
            $sql="INSERT INTO nnd_service (`serv_name`,`serv_name1`,`serv_desc`,`serv_img`,`serv_type`) VALUES( '$name',' $ename',' $content','$imgpath',$type)";
           $res=insert($sql);
           if($res['code']==1){
                echo "<script> alert('添加成功');window.location.href='server_list.php';</script>";
                                ...
                                <div class="col-md-7">
                                    <div class="form-group">
                                        <div class="input-group"><span class="input-group-addon">分类</span>
                                            <select name="serv_type" id="standard-list1" class="form-control">
                                                ...
                                                <?php foreach ($servtypes as  $v) {?>
                                                <option value="<?php echo $v['serv_type_id'] ?>"><?php echo $v['serv_type_name'] ?></option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="input-group"><span class="input-group-addon">标题</span>
                                            <input type="text" name="ser_type_name" value="" class="form-control">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="input-group"><span class="input-group-addon">英文标题</span>
                                            <input type="text" name="ser_type_ename" value="" class="form-control">
                                        </div>
                                    </div>
