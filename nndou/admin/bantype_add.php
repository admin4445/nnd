    <?php 
        ...
        if(!empty($_POST['sub']) && isset($_POST['sub'])){
            //获取表单post提交的数据
            $name=$_POST['bantype_name'];
            $add=$_POST['bantype_add'];
           
            //添加的sql语句
            $sql="INSERT INTO nnd_bantype (`btype_name`,`btype_add`) VALUES('$name','$add')";
            //调取封装的函数中的插入方法 ，返回的是数据的类型是布尔值 true或false
            $res=insert($sql);
            if($res['code']==1){
                echo "<script> alert('添加成功');window.location.href='bantype_list.php';</script>";
            }else{
                                ...
                                <div class="col-md-7">
                                    ...
                                    <div class="form-group">
                                        <div class="input-group"><span class="input-group-addon">类型名称</span>
                                            <input type="text" name="bantype_name" value="" class="form-control">
                                        </div>
                                    </div>
                                     <div class="form-group">
                                        <div class="input-group"><span class="input-group-addon">地址</span>
                                            <input type="text" name="bantype_add" value="" class="form-control">
                                        </div>
                                    </div>
